...
def traverseMaze(csmap, xs, ys, hs):
	visited = [(xs, ys)]
	stack = list()
	currx = xs
	curry = ys
	currh = hs
	csmap, stack = addNeighbsExplore(csmap, xs, ys, hs, stack, visited)
	...
	while len(stack)>0:
		...
		new_cmd = str(currh) + ',' + str(currx) + ',' + str(curry) + ',0,' + str(node[0]) + ',' + str(node[1])
		rospy.set_param('cmdlist', new_cmd)
		path = generate_path(csmap)
		follow_path(path)
		...
		visited.append(node)
		csmap, stack = addNeighbsExplore(csmap, currx, curry, currh, stack, visited)


def addNeighbsExplore(csmap, x, y, h, stack, visited):
	csmap = setWalls(x, y, h, csmap)
	n, e, s, w = getValidNeighbors(csmap, x, y)
	print 'neighbs'
	if n:
		if (x-1, y) not in visited:
...
def wander():
    ...
    rospy.set_param('curr_x',0)
    rospy.set_param('curr_y',0)
...
def setValidNeighbors(csmap, i, j, visited):
    ...
    if(csmap.horizontalWalls[i][j] or ([i-1, j] in visited)):
        if csmap.horizontalWalls[i][j]:
            rospy.loginfo("wall to the north")
        else:
            rospy.loginfo("visited north")
        n = False 
    if(csmap.verticalWalls[i][j+1] or ([i, j+1] in visited) ):
        if csmap.verticalWalls[i][j+1]:
            rospy.loginfo("wall to the east")
        else:
            rospy.loginfo("visited east")
        e = False
    if(csmap.horizontalWalls[i+1][j] or ([i+1, j] in visited)):
        if csmap.horizontalWalls[i+1][j]:
            rospy.loginfo("wall to the south")
        else:
            rospy.loginfo("visited south")
        s = False
    if(csmap.verticalWalls[i][j] or ([i, j] in visited)):
        if csmap.verticalWalls[i][j]:
            rospy.loginfo("wall to the west")
        else:
            rospy.loginfo("visited west")
        w = False         
    return n, e, s, w
